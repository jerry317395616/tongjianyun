...
匹配状态优先保存在缓存；旧站点若仍存在历史追踪 DocType，可兼容读取其 `erp_recipe_auto_mapping` 回执，但正式采购执行不依赖该旧表。采购预览复用结果，人工已确认的公司映射优先；毛料系数仍须核对。本流程不自动提交采购、库存或财务单据。

分类按 20 项分批，每批最多尝试 2 次，独立校验；失败批次不会丢弃其他成功批次。总计最多新增 5 个分类的限制仍跨批次生效。任务时限 1800 秒，覆盖最多 200 项的分批重试。回执保存脱敏后的批次编号、失败代码、食材 key 和尝试次数，不保存异常原文、密钥或模型原始响应。

零匹配且分类失败显示“同步失败”；存在成功匹配才显示“部分完成”；仅需业务核对则显示“待核对，尚未匹配”。权限不足、分类调用失败和模型主动要求人工核对分别提示。历史回执不会被本次部署改写；重新保存后产生新结果。
