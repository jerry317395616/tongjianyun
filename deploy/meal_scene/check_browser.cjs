...
(async()=>{
  ...
  try{
    ...
    if(await run(`document.querySelector('#stage').classList.contains('workbench')`)){
      ...
      check('marked lower strip and business menu are removed',await run(`!document.querySelector('.week-bottom,#more-work,#flow-nav')&&document.querySelector('#next-step')`));
      ...
      check('right panel retains recipe actions',await run(`!!document.querySelector('#panel-body [data-sub="recipe-new"]')&&!!document.querySelector('#panel-body [data-sub="recipe-import"]')`));
      ...
      await send('Emulation.setDeviceMetricsOverride',{width:1680,height:1050,deviceScaleFactor:1,mobile:false});await delay(250);
      await click('#workbench-week [data-workbench-date="2026-09-22"][data-workbench-meal="dinner"]');
      await until(`document.querySelector('#day').value==='2026-09-22'&&location.search.includes('2026-09-22')&&document.querySelector('#panel-body').textContent.includes('秋日食谱')`,'single-context week selection');
      check('clicking a meal changes the single business context',await run(`document.querySelector('#meal').value==='dinner'&&document.querySelector('#workbench-week .workbench-week-cell.selected')?.dataset.workbenchDate==='2026-09-22'`));
      check('missing meal remains explicitly unplanned',await run(`document.querySelector('#workbench-week [data-workbench-date="2026-09-22"][data-workbench-meal="dinner"]').textContent.includes('未编排')`));
      await run(`window.confirm=()=>true`);
      await click('#panel-body [data-sub="recipe-new"]');await until(`!!document.querySelector('#draft-title')`,'same-screen draft editor');
      check('editing uses persistent week and right-side form',await run(`document.querySelectorAll('#workbench-week .workbench-week-cell').length===25&&getComputedStyle(document.querySelector('#panel .recipe-week-scroll')).display==='none'`));
      await run(`(()=>{document.querySelector('#draft-title').value='统一工作台草稿';document.querySelector('#draft-dishes').value='杂粮饭';document.querySelector('#draft-dishes').dispatchEvent(new Event('input',{bubbles:true}));})()`);
      await click('#workbench-week [data-workbench-date="2026-09-23"][data-workbench-meal="lunch"]');
      check('selecting another week cell preserves prior draft content',await run(`document.querySelector('#workbench-week [data-workbench-date="2026-09-22"][data-workbench-meal="dinner"]').textContent.includes('杂粮饭')&&document.querySelector('#day').value==='2026-09-22'`));
      await click('#workbench-week [data-workbench-date="2026-09-22"][data-workbench-meal="dinner"]');
      await click('#draft-save');await until(`!!document.querySelector('#workbench-week')?.textContent.includes('杂粮饭')&&!document.querySelector('#draft-title')`,'draft saved');
      check('draft write is explicit and isolated',createdCount===1&&calls.filter(c=>c.write).length===1);
      await click('#panel-body [data-sub="recipe-import"]');await until(`!!document.querySelector('#recipe-import-file')`,'same-screen import');
      await run(`(()=>{const transfer=new DataTransfer();transfer.items.add(new File(['synthetic workbook'],'week.xlsx',{type:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'}));document.querySelector('#recipe-import-file').files=transfer.files;})()`);
      await click('#recipe-import-start');await until(`!!document.querySelector('#draft-title')&&document.querySelector('#panel-body').textContent.includes('Excel 识别结果')`,'import review');
      check('import stays in the week and requires an explicit save',await run(`document.querySelector('#week-caption').textContent.includes('正在编排草稿')&&document.querySelectorAll('#workbench-week .workbench-week-cell').length===25`)&&createdCount===1);
      await click('[data-sub="draft-cancel"]');await until(`!document.querySelector('#draft-title')`,'import cancel');
      await click('#next-step');await until(`document.querySelector('#panel-title').textContent==='食材采购'`);
      await click('#next-step');await until(`document.querySelector('#panel-title').textContent==='到货与入库'`);
      check('receipt stays in the right operation area',await run(`document.querySelector('#panel').open&&document.querySelector('#panel').getBoundingClientRect().width>250`));
      await click('#next-step');await until(`!!document.querySelector('#stock-warehouse')`);
      check('stock uses real read adapter',calls.some(c=>c.method==='get_stock'));
