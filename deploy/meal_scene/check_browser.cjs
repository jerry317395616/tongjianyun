...
(async()=>{
  ...
  try{
    ...
    if(await run(`document.querySelector('#stage').classList.contains('workbench')`)){
      ...
      check('eight real business entries remain',await run(`document.querySelectorAll('#flow-nav [data-step]').length===8`));
      check('unconfirmed groups are not counted as zero',await run(`document.querySelector('#facts').textContent.includes('待核对')`));
      ...
      await click('#week-new');await until(`!!document.querySelector('#draft-title')`,'same-screen draft editor');
      ...
      await click('#week-import');await until(`!!document.querySelector('#recipe-import-file')`,'same-screen import');
      ...
      await click('#flow-nav [data-step="receipt"]');await until(`document.querySelector('#panel-title').textContent==='到货与入库'`);
      ...
      await click('#flow-nav [data-step="stock"]');await until(`!!document.querySelector('#stock-warehouse')`);
