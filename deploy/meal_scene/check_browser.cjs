...
(async()=>{
  ...
  try{
    ...
    await click('#flow-nav [data-step="recipe"]');await until(`!!document.querySelector('[data-native="recipe"]')`);await click('[data-native="recipe"]');await until(`!!document.querySelector('iframe.professional')`);check('professional editor remains inside scene',await run(`location.pathname==='/tongjianyun-meal-scene'&&document.querySelector('iframe').getAttribute('src').startsWith('/desk/')`));await close();
