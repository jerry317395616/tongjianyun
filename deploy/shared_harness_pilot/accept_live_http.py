...
def main():
    ...
    try:
        for user in ['317395616@qq.com','harness.test.teacher.a@example.invalid']:
            stage = 'signed_login'
        ...
        for index,cookie in enumerate(cookies):
            ...
            assert status == 200
            visible = {row['sessionId'] for row in body['items']}
            assert sessions[index] in visible and sessions[1-index] not in visible
        ...
        ready = True  # User authorized live use with empty class data on 2026-09-07.
        print(json.dumps({'passed':ready,'protocol_passed':True,
            ...
            'data_scope_acceptance': 'empty_data_accepted_by_operator' if not visible_sample_count else 'sample_only',
