...
2026-09-25；第一阶段场景解耦已上线，第二阶段教师 Codex 候选验收中。

## 当前交付状态（优先于下文历史候选记录）

- 第一阶段随 `40f3d6c` 上线，`a5cdc29` 补充部署证据；真实教师在隔离浏览器完成
  本班点名、逐餐核对和刷新回读。其余专业业务、浏览器负例仍按各领域分别验收。
- 生产普通教师的右侧聊天仍明确未启用，不会运行管理员执行器。本轮新增工具代理与
  原生隔离执行器正在候选环境测试，不能称为教师 Codex 已上线。
- 用户要求**整个项目一套 Codex**：共用安装版本和统一入口，但各账号/任务独立会话
  与执行边界。“管理员项目”和“普通业务”是服务端权限通道，不是两套 AI 产品。
- 下文带“候选”“未部署”的段落保留为阶段设计与审计历史；已完成项以文末上线
  记录及 `business-codex-acceptance.md` 的新证据为准，不覆盖仍未完成的验收项。
