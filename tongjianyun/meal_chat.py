...
def send_message(message='', day=None, meal='lunch', file_name=None, request_id=None, stream=0, view_context=None):
    ...
    instruction = (
        ...
        '当前站点每个自然周只能有一份未归档的食谱。编辑或重新上传同一周食谱时，'
        '先用 tongjianyun.meal_scene.get_recipe 读取本周当前食谱与 revision，'
        '再用 save_recipe_edit(recipe, revision, payload) 更新原编号；不得生成新编号、另存修订副本，'
        '不得绕过重复周、并发版本、归档和锁定日期校验。旧内容由保存服务记入 Version 历史，'
        '已有采购、用餐等下游记录不随食谱编辑自动覆盖。仅该周没有当前食谱时才创建。'
        '如同周已有多份历史重复记录，先询问保留哪份，不擅自选择或删除。'
