...
function renderRecipeCalendar(){
  ...
  const edit=recipeCalendarState.edit,editAction=edit?.mode==='none'?note(edit.reason,'warning'):`<button class="secondary" data-sub="recipe-edit">编辑本周食谱</button>`;
...
async function demandView(){
  ...
  try{
    ...
    if(result.impact.requests.length){panelBody(note(result.impact.message,'warning')+note('该食谱已有关联采购需求，不重复生成；请核对原单，食谱修改不会自动改动已有采购需求。')+`<div class="buttons">${result.impact.requests.map(r=>button(r.name,'request',r.name)).join('')}</div>`);return;}
...
async function save(method,args){
  ...
  try{const result=await api(method,args,true);dirty=false;notify(method==='create_demand'?'采购需求草稿已保存':method==='create_recipe_draft'?'食谱草稿已保存':method==='save_recipe_edit'?'本周食谱已更新，历史版本已保留':'班级餐次记录已保存');return result;}
...
document.addEventListener('click',event=>{
  ...
  const step=event.target.closest('[data-step]');if(step){openStep(step.dataset.step);revealPanel();return;}
