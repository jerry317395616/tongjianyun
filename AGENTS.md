...
修改前检查源码、权限与影响范围，解析用户原始路由并锁定真实调用链。规划、路由解析及部署工具可用时优先使用，但不以指定工具调用作为硬性前置条件；工具不可用时，以等效的源码、配置和请求证据验证目标。

修改后运行最小相关测试；必要的资源构建和缓存更新在已授权任务范围内评估影响后执行。当前 DocType 结构仍冻结，不得顺带运行结构迁移。随后回到原始 URL 验证页面和网络请求；仅检查源码或 Python 返回值不能证明界面修改已生效。

保留全部已有未提交改动；只提交当前任务修改并同步到已核实的 GitHub 仓库和分支。普通 Harness 用户的受控执行、权限校验和审批边界不因本规则调整而放宽。
